...
class TestArm:
    def __init__(self) -> None:
        self.model = mujoco.MjModel.from_xml_path(f"./resources/arms/kinovagen3/mjcf/kinova_pan_toss.xml")
    ...
    def get_sensor_pita_pos(self):
        pita_pos = self.data.sensor(f"pita_pos").data
        print("sensor_pita_pos: ",pita_pos)
        return pita_pos
    
    def get_sensor_pan_mid_force(self):
        hf = self.data.sensor(f"pan_mid_force").data
        print("sensor_pan_mid_force: ".hf)
        return hf
